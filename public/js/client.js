...
let recStartTime;
let recElapsedTime;
let HyperchatTheme = "neon"; // neon - dark - forest - ghost ...
...
function getHtmlElementsById() {
  ...
  themeSelect = getId("HyperchatTheme");
...
function welcomeUser() {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      let message = {
        email: "",
        subject: "Please join our Hyperchat conference",
      ...
      switch (event.channel.label) {
        case "Hyperchat_chat_channel":
          try {
            ...
            console.error("Hyperchat_chat_channel", err);
          }
          break;
        case "Hyperchat_file_sharing_channel":
          try {
            let dataFile = msg.data;
            handleDataChannelFileSharing(dataFile);
          } catch (err) {
            console.error("Hyperchat_file_sharing_channel", err);
          }
          break;
      }
    };
...
/**
 * Set Hyperchat theme | dark | grey | ...
 * @param {string} theme type
 */
function setTheme(theme) {
  if (!theme) return;

  HyperchatTheme = theme;
  switch (HyperchatTheme) {
    case "neon":
      // neon theme
...
function createFileSharingDataChannel(peer_id) {
  fileDataChannels[peer_id] = peerConnections[peer_id].createDataChannel(
    "Hyperchat_file_sharing_channel"
  );
...
function showAbout() {
  ...
  Swal.fire({
    background: swalBackground,
    position: "center",
    title: "<strong>Hyperchat Made with ❤️ by Adel Eddarai</strong>",
    imageAlt: "About",
    imageUrl: aboutImg,
    html: `
        <br/>
        <div id="about">
            <b style="color:blue">Sponsored by Hyperlab</b> 
